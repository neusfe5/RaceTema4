...
# Usar la función "glimpse" sobre "cran"
glimpse(cran)
```
# dplyr
...
```{r}
subset(cran, country == "ES" & as.numeric(version) > 3.4)
```
...
```{r}
cran2 <- cran[, c("size", "r_version", "r_arch", "r_os", "package", "version", "country", "ip_id")]
```
...
```{r}
cran2[order(cran2$country, -as.numeric(cran2$r_version), cran2$ip_id), ]
```
...
cran %>%
  ...
  geom_point() +
  labs(
    title = "Tráfico de Información por País",
    x = "Código de País",
    y = "Tráfico Total (bytes)"
  ) +
  theme_minimal()
...
```{r}
code_paises <- read.csv("GeoLite2-Country-Locations-es.csv", encoding = "UTF-8")

cran <- cran %>% filter(!is.na(country))

code_paises <- code_paises %>% filter(!is.na(country_iso_code))

cran <- cran %>%
  left_join(code_paises %>% select(country_iso_code, continent_name), 
            by = c("country" = "country_iso_code"))

cran %>%
  group_by(continent_name) %>%
  summarise(total_traffic_tb = sum(size, na.rm = TRUE) / 2^40) %>%
  arrange(desc(total_traffic_tb)) %>%
  ggplot(aes(x = reorder(continent_name, -total_traffic_tb), y = total_traffic_tb)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(
    title = "Flujo de Datos por Continente",
    x = "Continente",
    y = "Tráfico Total (Terabytes)"
  ) +
  theme_minimal()
```
...
```{r}
students2 %>%
  pivot_longer(cols = starts_with("male_"), names_to = "sex_class", values_to = "count") %>%
...
students3_tidy <- students3 %>%
  pivot_longer(cols = starts_with("class"),names_to = "class", values_to = "score") %>%
  drop_na(score) %>%
  separate(col = class, into = c("class", "exam"), sep = "_") %>%
...
students3_tidy$class <- parse_number(students3_tidy$class)
...
sat_tidy <- sat %>%
  pivot_longer(
    cols = c("read_male", "read_fem", "math_male", "math_fem", "write_male", "write_fem"),
    names_to = c("part", "sex"),
    names_sep = "_",
    values_to = "count"
  )
...
```{r}
ggplot(sat_tidy, aes(x = score_range, y = count, color = sex, group = sex)) +
  geom_line() +
  facet_wrap(~part) +
  labs(
    title = "Diferencias por Sexo en las Puntuaciones SAT",
...
```{r}
dt2 <- c("2023-03-01", "2023-04-01", "2023-05-01")
ymd(dt2)
```
...
llegada_hk <- with_tz(llegada_ny, tzone = "Asia/Hong_Kong")
print(llegada_hk)

last_meeting <- ymd("2008-06-17", tz = "Asia/Singapore")
intervalo <- interval(last_meeting, now())
as.period(intervalo)
```
